// Карта разведки ИИ-видео (Salescast) — комнаты, люди, связи. Хранится в KV, НЕ в репозитории.
//
// Копия функции scout.js под отдельную карту: тот же гейт и тот же движок, но
// свой ключ KV, чтобы FF-карта и карта ИИ-видео не перезаписывали друг друга.
//
// Причина хранения в KV та же, что у scout.js и снимка дня /os/api/data:
// репозиторий портала публичный на GitHub, а здесь имена, телеграм-ники и
// дословные цитаты живых людей из закрытых чатов. Такому файлу в открытом
// репозитории не место. Путь /founder-finance/ закрыт Cloudflare Access.
//
//   PUT /founder-finance/api/scout-aivideo   — публикация. Только Bearer BOARD_READ_TOKEN
//                                              (ходит publish.py с Мака через pages.dev, вне гейта).
//   GET /founder-finance/api/scout-aivideo   — чтение. Access JWT (браузер) ИЛИ Bearer (Клод).
//
// Ключ KV: ff:scout-aivideo:latest.

import { json, authOk, bearerOk } from '../../os/api/_shared.js';

const KEY = 'ff:scout-aivideo:latest';
// Карта за две недели весит сотни килобайт: у каждого человека цитаты. Потолок
// держим с запасом, но не бесконечным — от заливки мусора.
...
export async function onRequestGet({ request, env }) {
  ...
  if (!raw) {
    return json({
      ...
      hint: 'запусти .scripts/telegram-scout-aivideo/run.sh на Маке — он собирает и публикует карту',
    }, 404);
  }
